...
type Config struct {
	...
	LingerDuration     time.Duration `yaml:"linger_duration"`
...
func (cfg *Config) RegisterFlagsWithPrefix(prefix string, f *flag.FlagSet) {
	...
	f.IntVar(&cfg.ConcurrentRequests, prefix+".concurrent-requests", 20, "The maximum number of concurrent requests to Kafka.")
	f.DurationVar(&cfg.LingerDuration, prefix+".linger-duration", time.Millisecond*50, "Time to wait before sending a request to kafka in order to fill the buffer.")
